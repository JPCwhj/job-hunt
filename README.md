...
2. **解析 JD**：提供岗位详情（任意招聘平台详情页截图，或用 Chrome 插件一键收藏后批量导出），AI 自动提取结构化信息（职位、薪资、要求、公司规模等）
...
| 场景 | 传统方式 | 用 job-hunt |
|---|---|---|
| 投 10 个岗位 | 一个一个研究 JD、改简历、写开场白，**预计 1 天** | 把 10 个岗位一次性发给 AI，**几分钟跑完 10 份定制材料** |
| 决定投谁 | 凭感觉 | 按匹配度排序，**数据驱动决策** |
| 评估标准 | 主观、易疲劳 | AI 用同一套 STAR 标准，**一致性高** |

你发多少个岗位，AI 就给你产出多少份定制简历——5 个、10 个、20 个都行。
...
如果你不想截图，可以装一个浏览器插件直接收藏岗位再批量导出。**已支持 Boss 直聘、前程无忧、智联招聘、猎聘四个平台**。
...
- 岗位来源任选其一：
  - **截图模式**：任意招聘平台详情页截图（Boss直聘、智联招聘、前程无忧、猎聘、拉勾等均可）；要求你的大模型具备视觉能力
    - 📌 一张截图 = 一个岗位；信息长就用长截图截全；截图里**至少要有岗位名称**（公司名可选）
  - **Chrome 插件模式**：收藏岗位后导出 `.jobs.json` 文件，对大模型无视觉要求（详见上方「安装浏览器插件」）
...
- **支持所有主流招聘平台**：截图模式只要包含公司名、职位名、岗位 JD 即可，不限平台；Chrome 插件目前覆盖 Boss 直聘、前程无忧、智联招聘、猎聘
- **不做自动投递**，避免封号风险
- **两种导入方式各有适用场景**：
  - **截图模式**：零依赖，无需任何浏览器插件或 MCP 工具，但要求你的大模型具备视觉能力（能识图）
  - **Chrome 插件模式**：直接读取页面 DOM 导出结构化 JSON，对大模型无视觉要求；插件为可选增强，不装也能完整跑通
