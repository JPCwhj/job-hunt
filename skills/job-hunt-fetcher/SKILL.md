...
写完 jd-pool 文件后，**必须**展示每个岗位的关键识别字段，让用户确认识别是否准确。**整条消息以「👉 回复...」结尾，停止执行等待用户回复**。

输出格式（按下面这段，整条就是回复的全部内容）：

```
📋 已识别 <N> 个岗位，请确认信息是否正确：

【1】<公司名> · <职位名>
   💰 <salary.range>（<monthly_count 薪>，若 null 省略）
   📍 <location.city> · <location.district>
   👤 经验：<requirements.experience> · 学历：<requirements.education>
   🏷️ 标签：<tags 前 3 个，逗号分隔>
   <若有缺失字段>⚠️ 未识别到：<缺失字段中文名列表>
...
👉 如果识别正确，回复「确认」或「继续」；
   如果某条有误，告诉我哪一条哪里错了（例如「第 2 条公司名错了，应该是 XXX」「第 3 条薪资是 30-50K」），我来更正。
```

### 用户回复处理

- **回「确认」/「继续」/「对」/「ok」等肯定意图** → 跳过更正，进入 Step 4 返回
- **指出具体错误**（如「第 N 条公司名错了，应该是 XXX」「第 2 条薪资是 30-50K」）：
  1. 定位到对应的 jd-pool 文件
  2. 用 Edit / Write 工具更正 frontmatter 中的对应字段
  3. 重新输出本 Step 3 的确认列表（含更正后的内容）
  4. 再等用户回复，直到用户说「确认」
- **指出某条整条不对**（如「第 N 条删掉」）→ 删除对应 jd-pool 文件，重新输出确认列表
- **回复模糊** → 简短复述当前列表，再询问一次
