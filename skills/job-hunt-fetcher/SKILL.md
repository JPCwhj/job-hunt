...
**分支 B：某些截图缺 `title`**（这是用户截图不全的典型情况，必须问用户补）

整条消息以「👉 回复...」结尾：

```
📋 我看了你发的 <N> 张截图，但其中 <M> 张没识别到职位名：

- 第 <X> 张：JD 内容是「<JD 前 30 字预览>...」 ❓ 没识别到职位名
- 第 <Y> 张：JD 内容是「<JD 前 30 字预览>...」 ❓ 没识别到职位名

请按编号告诉我每张截图对应的职位名，例如：
「第 1 张：高级产品经理；第 2 张：数据分析师」

👉 回复每张截图的职位名
```

用户回复后，把用户提供的 title 写入对应 JD，进入 Step 2。
