...
1. **优先**：调用 Skill 工具加载 `docx` skill 解析（内部走 pandoc，转为 Markdown，保留章节标题、列表、表格、加粗等结构）
2. **降级**（仅当当前会话的可用 skill 列表中**不包含** `docx` 时）：告知用户：

   「未检测到 `docx` skill，无法直接解析 Word 简历。请改用以下任一方式：
   ① 用 Word/WPS 打开简历，另存为 `.md` 或 `.pdf` 后重发
   ② 全选简历内容复制，直接粘贴到消息框发送」

   停止执行，等待用户重新操作

判断依据：当前会话的可用 skill 列表中包含 `docx` 即用方案 1，否则走方案 2 提示用户。
