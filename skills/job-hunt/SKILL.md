...
1. **优先**：调用 Skill 工具加载 `pdf` skill 解析，提取文本内容并转为 Markdown（保留章节标题、列表、表格结构）。`pdf` skill 基于 `pypdf` 等 Python 库做纯文本提取，不依赖模型视觉能力，适用于所有 LLM
2. **降级**（仅当当前会话的可用 skill 列表中**不包含** `pdf` 时）：用 Read 工具直接读取 PDF 文件（Claude Code 内建支持，依赖模型视觉能力；大于 10 页时需 `pages` 参数分段读取，单次最多 20 页），将读到的内容整理为 Markdown
3. 解析完成后，向用户简短确认：「PDF 简历已解析。如发现关键内容缺失或错位，可贴文本或转 Markdown 后重发。」继续进入 Step 2.5 评估，不需用户回复

判断依据：当前会话的可用 skill 列表中包含 `pdf` 即用方案 1，否则降级到方案 2。
