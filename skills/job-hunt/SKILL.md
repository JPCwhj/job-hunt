...
**关键：问题必须是本轮回复的最后一段，前面不要放任何长预览或额外说明，让用户视线落到的最后一行就是「回复 A 或 B」**。

输出格式严格按下面这段（**无任何前置说明、无任何文本预览、无 Bash 输出回显**，整段就是回复的全部内容）：

```
📝 收到你的简历。粘贴的文本看起来格式有些问题——可能存在断行、缺章节标记等，常见于从 PDF/Word 复制带过来的。
...
**禁止**：在这条问题之后再追加任何其他文字（如"请尽快回复""我等你"等），最后一行必须是「👉 回复 A 或 B」。
