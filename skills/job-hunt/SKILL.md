...
- **发文件（.pdf）**：调用 Skill 工具加载 `pdf` skill 解析，提取文本内容并转为 Markdown 文本（保留章节标题、列表、表格结构）
- **发文件（.md）**：直接读取文件内容
- **发文件（其他格式）**：告知用户「暂时只支持 .md、.docx、.doc、.pdf 格式的简历文件。如果是其他格式，请将文字粘贴发给我。」停止执行，等待用户重新操作
- **文件路径（.docx / .doc）**：用 Bash 确认路径存在后，调用 `docx` skill 解析
- **文件路径（.pdf）**：用 Bash 确认路径存在后，调用 `pdf` skill 解析，提取文本内容并转为 Markdown 文本
- **文件路径（.md）**：用 Bash 读取文件内容
- **文件路径（其他格式）**：告知用户「暂时只支持 .md、.docx、.doc、.pdf 格式的简历文件。如果是其他格式，请将文字粘贴发给我。」停止执行，等待用户重新操作
- **粘贴文本**：直接使用该文本内容

**PDF 解析注意**：PDF 是版式文件，解析后可能丢失部分排版（多栏布局、文本框、图标）。解析完成后，向用户简短确认：「PDF 简历已解析。如发现关键内容缺失或错位，可贴文本或转 Markdown 后重发。」继续进入 Step 2.5 评估，不需用户回复。
